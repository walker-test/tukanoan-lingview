...
export function setupTextSync() {
  ...
  window.sync = function sync(current_time) {
    for (var i = 0; i < ts_tag_array.length; i++) {
      ...
      if (
        current_time - 0.001 >= parseFloat(ts_start_time_array[i]) / 1000.0 &&
        current_time <= parseFloat(ts_stop_time_array[i]) / 1000.0
      ) {
  ...
  function updateTimestampQuery(newSentenceIndex) {
    if (window.history.replaceState) {
      ...
      const newurl =
        window.location.href.replace(/\?.*$/, "") + `?${newSentenceIndex}`; // hacky...
