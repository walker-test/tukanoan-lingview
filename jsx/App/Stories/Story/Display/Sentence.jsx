...
function Row({ numSlots, values, tier }) {
  ...
  for (const v of values) {
    ...
    output.push(
      <td key={id.generate()} colSpan={size}>
        {text}
      </td>
    );
    currentSlot = endSlot;
  ...
  if (sentence["noTopRow"] == null || sentence["noTopRow"] === "false") {
    rowList.push(
      <tr data-tier={htmlEscape(sentence["tier"])}>
        <td colSpan={numSlots} className="topRow">
          {sentence["text"]}
        </td>
      </tr>
    );
        ...
        <thead>
          <tr>
            <td>
              <b>
                {" "}
                <TranslatableText dictionary={storySearchText} />
              </b>
              : {title}
            </td>
          </tr>
